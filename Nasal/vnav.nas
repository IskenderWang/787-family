...
var vnav = {
    ...
    update : func {
        ...
        if (
            (getprop("/autopilot/locks/altitude") == "vnav")
            and (getprop("/autopilot/vnav/vnav-mode") == "crz-altitude-hold")
        ) {
            setprop("/autopilot/locks/vnav", "crz");
        } elsif (
            (getprop("/autopilot/locks/altitude") == "vnav")
            and (getprop("/autopilot/vnav/vnav-mode") == "wp-mode")
        ) {
            setprop("/autopilot/locks/vnav", "wp");
        } else {
            setprop("/autopilot/locks/vnav", "");
        }
        ...
        if (
            getprop("autopilot/vnav/vnav-mode") == "wp-mode"
            and getprop("controls/navigation/vnav/wp1alt") != nil
        ) {
            if (getprop("controls/navigation/vnav/wp1alt") <= 0) {
                ...
                if ((targetaltft != nil) and (getprop("autopilot/route-manager/wp/dist") != nil)) {
                    setprop(
                        "autopilot/vnav/vertical-speed-fps",
                        (getprop("velocities/groundspeed-kt") * (targetaltft - altitudeft))
                        / (3600 * getprop("autopilot/route-manager/wp/dist"))
                    );
