...
#
# Modified for 787 by IskenderWang
...
var _start = func() {
	...
	if (!_timer.isRunning) {
		...
		if ((getprop("/gear/gear[0]/wow")) and ((getprop("/velocities/groundspeed-kt")) < 35)) {
			setprop("/sim/model/autopush/available", 1);
		} else {
			setprop("/sim/model/autopush/available", 0);
		}

		# Prevents pushback with ground equipment connected
		if (getprop("services/ext-pwr/enable") or getprop("services/fuel-truck/enable") or getprop("services/baggage/enable") or getprop("services/deicing_truck/enable") or getprop("services/catering/enable") and (getprop("sim/model/autopush/target-speed-km_h") != 0)) {
